    ...
    <div class="flex flex-col items-center justify-center pt-16 pb-4 bg-gray-200">
        ...
        <div class="container mx-auto bg-white shadow-md rounded-lg p-6 w-3/4">
            <div class="mb-4 bg-gray-200 shadow-md rounded-lg p-6">
                <label class="block text-sm font-medium text-gray-700">Select Corporation</label>
                <select wire:model="corporation_id" wire:change="onCorporationSelected($event.target.value)" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                    <option value="">-- Select Corporation --</option>
                    @foreach($corporations as $pcorp)
                        <option value="{{ $pcorp->id }}">{{ $pcorp->name }}</option>
                    @endforeach
                </select>
                <div wire:loading wire:target="corporation_id">
                    <span class="text-green-500 m-2">Loading Coprporation ...</span>
                </div>   
            </div>
            ...
            @if($selectedCorporation)  
                ...
                <div class="container mx-auto bg-white shadow-md rounded-lg p-6 w-full">
                    <form wire:submit.prevent="{{ $locationId ? 'updateLocation' : 'createLocation' }}" class="space-y-4 pb-4">
                        <div class="mb-4">
                        ...
                        <div class="w-full flex gap-2">
                            <button type="submit"
                                class="{{ $locationId ? 'w-1/3' : 'w-1/3' }} py-2 text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition">
                                {{ $locationId ? 'Update' : 'Save' }} Location
                            </button>
                            
                            @if ($locationId)
                                <button type="button" wire:click="onCancelLocation"
                                        ...
                                        <tr class="border-b border-gray-200 bg-gray-100 hover:bg-gray-200 hover:border-gray-300">
                                            ...
                                            <td class="py-3 px-6 text-left">
                                                <button wire:click="toggleActive({{ $location->id }})"
                                                        class="px-3 py-1 text-white rounded {{ $location->is_active ? 'bg-green-500' : 'bg-gray-500' }}">
