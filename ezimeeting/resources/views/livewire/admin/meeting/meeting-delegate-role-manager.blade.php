<div>
    <div class="flex flex-col items-center justify-center pt-16 pb-4 bg-gray-200">
        ...
        <div class="container mx-auto bg-white shadow-md rounded-lg p-6 w-full">
            <form wire:submit.prevent="{{ $roleId ? 'updateRole' : 'createRole' }}" class="space-y-4 pb-4">
                <div class="mb-4">
                ...
                <div class="w-full flex gap-2">
                    <button type="submit"
                        class="{{ $roleId ? 'w-1/3' : 'w-1/3' }} py-2 text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition">
                        {{ $roleId ? 'Update' : 'Save' }} Role
                    </button>
                    
                    @if ($roleId)
                        <button type="button" wire:click="onCancelRole"
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        @foreach ($roles as $role)
                            <tr class="border-b border-gray-200 bg-gray-100 hover:bg-gray-200 hover:border-gray-300">
                                ...
                                <td class="py-3 px-6 text-left">{{ $role->text }}</td>
                                
                                {{--
                                <td class="py-3 px-6 text-left">
                                    <button wire:click="toggleActive({{ $role->id }})"
                                            class="px-3 py-1 text-white rounded {{ $role->is_active ? 'bg-green-500' : 'bg-gray-500' }}">
